...
def main():
    ...
    parser.add_argument(
        "command",
        choices=["index", "create_wheel", "install_develop", "git_status", "git_gui", "git_pull_origin"],
        default="index", type=unicode, help="What command to execute."
    )
    parser.add_argument( "root_directory",
        help="The directory that contains all the python package repositories." )
    parser.add_argument( "--site_packages", default=None,
    ...
    parser.add_argument( "--git_fp", default="git",
    ...
    if args.command == "create_wheel":
        ...
        if args.project is None:
            raise ValueError( "'--project' must be set when using the 'install_develop' command." )
        if args.site_packages is None:
            raise ValueError( "'--site_packages' must be set when using the 'install_develop' command." )
        pip_repo_manager.install_develop( args.project, args.site_packages )
